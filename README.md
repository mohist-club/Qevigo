...
本项目是 [mohist-club/Poptro](https://github.com/mohist-club/Poptro)（MIT）v1.6.0 的重构版。功能保持对齐，
...
./scripts/build-app.sh              # 生成 dist/Poptro.app（release，ad-hoc 签名）
CONFIG=debug ./scripts/build-app.sh # 调试构建，更快
./scripts/make-dmg.sh               # 生成 dist/Poptro.dmg 与 dist/Poptro.zip
open dist/Poptro.app
```

首次运行需要在 系统设置 → 隐私与安全性 → 辅助功能 中授权。
授权与 App 路径绑定：日常使用请把 `Poptro.app` 放到 `/Applications` 后再授权，避免反复授权。
...
```
Sources/PoptroCore/       无 UI 的核心库（可单测）
...
数据在 `~/Library/Application Support/Poptro/v2/`（与原版目录分开，两者可并存）。
API Key 用 AES-GCM 加密后保存（密钥由本机硬件 UUID 派生），文件权限 0600。这样可避免明文落盘，
也避免 ad-hoc 签名的 App 反复弹钥匙串授权；但它不等同于钥匙串或硬件安全模块。

环境变量 `POPTRO_DATA_DIR` 可改数据目录（测试用）。

## 开发辅助

```bash
# 把每个设置标签页渲染成 PNG（进程内渲染，不需要屏幕录制权限）
dist/Poptro.app/Contents/MacOS/Poptro --snapshot /tmp/shots [--english] [--dark]
# 用固定文本打开翻译浮窗并截图
POPTRO_DATA_DIR=/tmp/data dist/Poptro.app/Contents/MacOS/Poptro --demo-panel "Hello" /tmp/panel.png
```
...
`AppInfo.updateRepository` 为 `nil` 时，「检查更新」相关界面自动隐藏。发布到自己的 GitHub 仓库后，
把它设为 `"owner/repo"` 即可启用（读取 Releases，下载 DMG，手动替换）。
