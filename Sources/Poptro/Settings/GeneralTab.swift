            ...
            Form {
                ...
                LabeledContent(tr("窗口玻璃效果", "Window Glass Effect")) {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        FormNote(tr("外观设置适用于所有 Poptro 窗口。", "Appearance applies to all Poptro windows."))
                    }
                }
                ...
                LabeledContent(tr("所需权限", "Required Access")) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        FormNote(tr(
                            "Poptro 需要「辅助功能」来读取选中的文字并响应快捷键，不会记录或上传你的内容。",
                            "Poptro needs Accessibility to read selected text and respond to shortcuts. It never records or uploads your content."
                        ))
                    }
                }
